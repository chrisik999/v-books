...
import { createBook, listBooks, getBookById, updateBookById, deleteBookById, deleteBooksByIds } from "../services/book.service";
...
export async function createBookHandler(req: Request, res: Response) {
  ...
  try {
    const doc = await createBook({
      ...
      imagePath: image ? path.join("uploads", "images", path.basename(image.path)) : undefined,
      pdfPath: pdf ? path.join("uploads", "pdfs", path.basename(pdf.path)) : undefined,
    } as any);
    return res.status(201).json({ book: doc });
  } catch (err: any) {
    const msg = err?.code === 11000 ? "isbn already exists" : err?.message || "Failed to create book";
...
export async function updateBookHandler(req: Request, res: Response) {
  ...
  if (image) updates.imagePath = path.join("uploads", "images", path.basename(image.path));
  if (pdf) updates.pdfPath = path.join("uploads", "pdfs", path.basename(pdf.path));

  try {
    const updated = await updateBookById(id, updates);
    return res.json({ book: updated });
  } catch (err: any) {
    const msg = err?.code === 11000 ? "isbn already exists" : err?.message || "Failed to update book";
    const status = err?.code === 11000 ? 409 : 400;
    return res.status(status).json({ error: msg });
  }
}
...
export async function deleteBooksManyHandler(req: Request, res: Response) {
  ...
  if (role !== "ADMIN") {
    ...
    const own = await Promise.all(ids.map(async (id) => {
